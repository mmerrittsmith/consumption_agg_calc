...
def clean_mrk_mod_d(mrk_mod_d: pd.DataFrame, col_names: dict) -> pd.DataFrame:
    ...
    mrk_mod_d = mrk_mod_d[mrk_mod_d["item_available"] == "Yes"]
...
def standardize_units(hh_mod_g1: pd.DataFrame, mrk_mod_d: pd.DataFrame) -> pd.DataFrame:
    ...
    and another containing market data with unit weights (mrk_mod_d). It merges these
    ...
    Note:
        This function assumes that the units to kilograms ratio is homogeneous across the nation.
